...
def install(config: ZfsSystemConfig):
    ...
    subprocess.run(f'zfs snapshot -r {rpool_nix}@install_start'.split(),
                   check=True)
    subprocess.run(f'zfs snapshot -r {bpool_nix}@install_start'.split(),
                   check=True)

    nixos_install = 'nixos-install -v --show-trace --no-root-passwd --root /mnt'
    subprocess.run(nixos_install.split(), check=True)

    subprocess.run(f'zfs snapshot -r {rpool_nix}@install'.split(),
                   check=True)
